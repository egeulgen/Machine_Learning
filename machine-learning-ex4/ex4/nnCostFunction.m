...
for i = 1:m
    ...
    for k = 1:num_labels    
        temph = h(i,k);
        J = J + tempy(k)*log(temph)+(1-tempy(k))*log(1-temph);
    end
    delta3 = h(i,:)' - tempy;
...
reg_term1 = Theta1(:,2:end).^2;
reg_term1 = sum(reg_term1(:));

reg_term2 = Theta2(:,2:end).^2;
reg_term2 = sum(reg_term2(:));
